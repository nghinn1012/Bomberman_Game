...
import static uet.entities.item.SpeedItem.*;

import uet.entities.animal.*;
import uet.graphics.Sprite;

public class Move {

    public static void checkRun(Animal animal) {
        // nếu aminal check run là bomber thì trừ count + 
    ...
    private static void setDirection(String direction, Animal animal, int isMove) {
        // các trường hợp direction
        switch (direction) {
            // các trường hợp đi xuống đi lên tuỳ isMove
            case "down":
                down_step(animal);
